...
export async function sendCatalogLink(to: string) {
  ...
  try {
    ...
    console.warn("[WhatsApp] Template 3p_direct_integration_test_template failed, falling back to dynamic CTA button.");
...
export async function sendCheckoutLink(to: string, orderId: string, total: number, itemsNames: string) {
  ...
  try {
    ...
    console.warn("[WhatsApp] Template secure_checkout failed, falling back to dynamic CTA button.");
    // Fallback: send dynamic interactive cta_url button
    await sendCtaUrlButton(
      to,
